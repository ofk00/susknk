...
.addField('Ad', mention.tag+` (${mention})`, true)
...
.addField('Sunucuya Katılma Tarihi', `${moment(mentionMember.joinedAt).format("LL")} | (${moment(mentionMember.joinedAt).fromNow()})`, true)
.addField('Discord Katılma Tarihi', `${moment(mention.createdAt).format("LL")} | (${moment(mention.createdAt).fromNow()})`, true)
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['profil', 'userinfo', 'kullanıcıbilgi', 'user'],
  permLevel: 0
};

...
if (!args.length) {
            axios.get('https://corona.lmao.ninja/v2/all')
                .then((response) => {
                    const exampleEmbed = new Discord.MessageEmbed()
                        ...
                        .setDescription('Ülkeler hakkında bilgi almak için ' + ` \`${prefix}korona <ülke>\` `)
                        .addField(`Toplam Tanı Konulan Hasta Sayısı`,response.data.cases)
          ...
          console.log(response)
                ...
                .catch((error) => {
          message.channel.send(':x: Hata \n Lütfen ülke girerken global olarak giriniz. Örnek: Turkey')
                    console.log(error);
                });
        }
}
